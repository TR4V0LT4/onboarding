...
export default async function handler(request: NextRequest) {
 try {   
    ...
    if (findUser) {
      return NextResponse.json({success: false, type: "email", message: "Cet e-mail est déjà utilisé."}, {status: 401});
    }
    const checkUser: any = (await db.select().from(users).where(eq(users.phone, user.phone)))[0];
    if (checkUser) {
      return NextResponse.json({ success: false, type: "phone", message: "Ce numéro de téléphone est déjà utilisé." }, { status: 401 });
    }
    const checkPharmacy = await findPharmacyByIce(user.ice);
    if (checkPharmacy) {
      return NextResponse.json({ success: false, type: "ice",  message: "Cette pharmacie existe déja. Veuillez contacter le responsable de la pharmacie pour vous accorder l'accès. Dr " + user.first_name + ". \n Si vous notez un problème de titualire veuillez appeler le 05 22 27 37 47 ou nous contacter sur contact@blinkpharma.ma" }, { status: 401 });
    }
    const hashedPassword = await bcrypt.hash(user.password, 10);
    const code = await sendOTP(user.phone);
    if (!code) {
      return NextResponse.json({ success: false,  message: "Erreur lors de l'envoi du code OTP.", type: "ice" }, { status: 401 });
    }
    const pharmacy: any = await db.insert(companies).values({
      id: uuidv4(),
    ...
    await createOngoingOffer(pharmacy.id);
    ...
    const newUser =  await db.insert(users).values({
      ...
      code: code as string,
      // company_id: pharmacy['id'] as string,
    ...
    return NextResponse.json({ success: true, message: newUser, is_completed: true });
  }
  catch(error) {
